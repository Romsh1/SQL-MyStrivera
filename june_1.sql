...
SELECT 
	e.employee_id, e.first_name, e.last_name, e.hire_date, r.region, r.country
FROM regions r
	JOIN employees e
...
SELECT
	e.employee_id, e.first_name, e.email, d.department, d.division
FROM employees e
	JOIN departments d 
...
GROUP BY region_id, region
	HAVING employee_cnt = 0;

-- 10. Compare average salaries of employees hired before and after 2010 by department.
SELECT 
	ROUND(AVG(CASE WHEN EXTRACT(YEAR FROM e.hire_date) < 2010 THEN (e.salary) END), 2) AS avg_salary_before_2010,
	ROUND(AVG(CASE WHEN EXTRACT(YEAR FROM e.hire_date) >= 2010 THEN (e.salary) END), 2) AS avg_salary_after_2010,
    d.department
FROM employees e
	JOIN departments d
ON e.department = d.department
...
-- 13. List departments with average salary below 90,000 and more than 40 employees.
SELECT d.department, ROUND(AVG(salary), 2) AS avg_salary
FROM departments d JOIN employees e ON d.department = e.department
GROUP BY department
HAVING avg_salary < 90000 AND COUNT(employee_id) > 40;
